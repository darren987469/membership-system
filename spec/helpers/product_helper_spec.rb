...
describe ProductHelper do
  ...
  describe '#product_default_image(product, resize: nil)' do
    let!(:product) { create(:product, images: images) }

    context 'product has image' do
      let(:images) { [] }

      it { expect(product_default_image(product)).to be_nil }
    end
    ...
    context 'product has no image' do
      ...
      it 'returns resized image variant of the product' do
        expect(product_default_image(product)).to be_a ActiveStorage::Variant
      end
    end
  end
end
